...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('yn-upms-systemid', systemid);
		$.cookie('yn-upms-systemname', systemname);
		$.cookie('yn-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('yn-upms-systemid') || 1;
	var systemname = $.cookie('yn-upms-systemname') || 'yn-upms-server';
	var systemtitle = $.cookie('yn-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);

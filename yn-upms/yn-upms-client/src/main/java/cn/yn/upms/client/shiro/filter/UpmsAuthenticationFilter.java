package cn.yn.upms.client.shiro.filter;

import com.alibaba.fastjson.JSONObject;
import cn.yn.common.util.PropertiesFileUtil;
import cn.yn.common.util.RedisUtil;
import cn.yn.upms.client.shiro.session.UpmsSessionDao;
import cn.yn.upms.client.util.RequestParameterUtil;
import cn.yn.upms.common.constant.UpmsConstant;
...
/**
 * 重写authc过滤器
 * Created by wangjianlong on 2017/3/11.
 */
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    // 局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_ID = "yn-upms-client-session-id";
    // 单点同一个code所有局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_IDS = "yn-upms-client-session-ids";
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        // 判断请求类型
        String upmsType = PropertiesFileUtil.getInstance("yn-upms-client").get("yn.upms.type");
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        StringBuffer sso_server_url = new StringBuffer(PropertiesFileUtil.getInstance("yn-upms-client").get("yn.upms.sso.server.url"));
        // server需要登录
        String upmsType = PropertiesFileUtil.getInstance("yn-upms-client").get("yn.upms.type");
        if ("server".equals(upmsType)) {
        ...
        sso_server_url.append("/sso/index").append("?").append("appid").append("=").append(PropertiesFileUtil.getInstance("yn-upms-client").get("yn.upms.appID"));
        // 回跳地址
